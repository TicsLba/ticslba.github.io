...
public class PrivacyNoticeWindow:Window
{
 ...
 public static string FlagFile{get{var d=Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),"PANGI");Directory.CreateDirectory(d);return Path.Combine(d,"privacy-notice-2.1.ok");}}
 public static bool Seen=>File.Exists(FlagFile);
 public PrivacyNoticeWindow(){Title="PANGI · Privacidad, responsabilidad y seguridad";Width=780;Height=760;MinWidth=680;MinHeight=640;WindowStartupLocation=WindowStartupLocation.CenterScreen;ResizeMode=ResizeMode.CanResize;Background=Bg;FontFamily=new FontFamily("Segoe UI");Build();}
 ...
 void Build(){
  ...
  var names=new StackPanel{Margin=new Thickness(14,0,0,0),VerticalAlignment=VerticalAlignment.Center};names.Children.Add(T("PANGI",30,true,Navy));names.Children.Add(T("Gestión, informes y recuperación",16,true,Blue));brand.Children.Add(names);root.Children.Add(brand);
  var intro=T("PANGI administra equipos institucionales, identifica al responsable de cada sesión y genera informes de uso. La supervisión de pantalla sigue siendo en tiempo real y no crea grabaciones históricas por defecto.",15,false,Muted);intro.Margin=new Thickness(0,18,0,14);root.Children.Add(intro);
  ...
  b.Click+=(_,_)=>{File.WriteAllText(FlagFile,"PANGI 2.1 privacy notice acknowledged");DialogResult=true;};root.Children.Add(b);
  var dev=T("PANGI 2.1 · Gestión institucional de tablets",12,false,Muted);dev.TextAlignment=TextAlignment.Center;dev.Margin=new Thickness(0,8,0,12);root.Children.Add(dev);Content=sc;
 }
}
