...
public sealed class TeacherConsoleWindow : Window
{
    ...
    void RebuildWall()
    {
        ...
        if (list.Count == 0)
        {
            ...
            empty.Children.Add(Txt("Verifica que las tablets estén en la misma red, con la misma clave técnica y con PANGI activo.", 13, Muted));
    ...
    async Task Send(string action, string value = "")
    {
        ...
        if (list.Count == 0)
        {
            MessageBox.Show("Selecciona al menos una tablet.", "PANGI", MessageBoxButton.OK, MessageBoxImage.Information);
            return;
        }
    ...
    void OpenMosaic()
    {
        ...
        if (list.Count == 0)
        {
            MessageBox.Show("Selecciona tablets que tengan una sesión supervisada activa.", "PANGI", MessageBoxButton.OK, MessageBoxImage.Information);
            return;
        }
