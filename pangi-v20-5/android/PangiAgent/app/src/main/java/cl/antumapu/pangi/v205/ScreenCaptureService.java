...
public class ScreenCaptureService extends Service {
    ...
    void write(Socket c,int code,String type,byte[] b)throws IOException{OutputStream o=c.getOutputStream();String h="HTTP/1.1 "+code+" OK\r\nContent-Type: "+type+"\r\nX-PANGI-Encryption: aes-256-gcm\r\nContent-Length: "+b.length+"\r\nConnection: close\r\nCache-Control: no-store, no-cache, must-revalidate\r\nPragma: no-cache\r\n\r\n";o.write(h.getBytes(StandardCharsets.US_ASCII));o.write(b);o.flush();}
    Notification note(){NotificationManager nm=(NotificationManager)getSystemService(NOTIFICATION_SERVICE);String c="capture";if(Build.VERSION.SDK_INT>=26&&nm.getNotificationChannel(c)==null)nm.createNotificationChannel(new NotificationChannel(c,"PANGI · supervisión",NotificationManager.IMPORTANCE_LOW));Notification.Builder b=Build.VERSION.SDK_INT>=26?new Notification.Builder(this,c):new Notification.Builder(this);Intent open=new Intent(this,MainActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK|Intent.FLAG_ACTIVITY_CLEAR_TOP|Intent.FLAG_ACTIVITY_SINGLE_TOP);PendingIntent pi=PendingIntent.getActivity(this,301,open,PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE);return b.setContentTitle("PANGI · supervisión activa").setContentText("Supervisión en tiempo real, no vigilancia histórica").setSmallIcon(android.R.drawable.presence_online).setContentIntent(pi).setOngoing(true).setOnlyAlertOnce(true).build();}
}
