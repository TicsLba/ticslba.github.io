...
public class PrivacyGateActivity extends Activity{
 ...
 void showNotice(){
  ...
  LinearLayout mark=new LinearLayout(this);mark.setGravity(Gravity.CENTER);mark.setBackground(shape(BLUE,18));ImageView icon=new ImageView(this);icon.setImageResource(R.drawable.pangi_mark);mark.addView(icon,new LinearLayout.LayoutParams(dp(48),dp(48)));brand.addView(mark,new LinearLayout.LayoutParams(dp(58),dp(58)));
  LinearLayout names=new LinearLayout(this);names.setOrientation(LinearLayout.VERTICAL);names.setPadding(dp(14),0,0,0);names.addView(t("PANGI",26,true,NAVY));names.addView(t("Privacidad y seguridad de la sesión",13,false,MUTED));brand.addView(names);root.addView(brand);
  ...
  TextView foot=t("PANGI 4.0 · Gestión de aula y dispositivos",12,false,MUTED);foot.setGravity(Gravity.CENTER);foot.setPadding(0,dp(12),0,0);root.addView(foot);setContentView(sc);
 }
