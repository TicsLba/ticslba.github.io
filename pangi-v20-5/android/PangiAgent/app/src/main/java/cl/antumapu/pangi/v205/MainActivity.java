...
public class MainActivity extends Activity {
    ...
    static final int COBALT=Color.rgb(24,167,124),NAVY=Color.rgb(17,24,32),TEAL=Color.rgb(90,216,177),
            SUN=Color.rgb(242,184,75),CORAL=Color.rgb(231,101,101),VIOLET=Color.rgb(18,56,68),
            INK=Color.rgb(245,247,247),MUTED=Color.rgb(168,184,185),BG=Color.rgb(17,24,32),
            LINE=Color.rgb(45,61,68),WHITE=Color.WHITE;
    ...
    void route(){
        if(Managed.profileOwner(this)){
            ...
            if(s==SessionState.State.ACTIVE){
                ...
                // PANGI se reinicie o MediaProjection se interrumpa.
    ...
    LinearLayout card(){LinearLayout c=new LinearLayout(this);c.setOrientation(LinearLayout.VERTICAL);c.setPadding(dp(20),dp(18),dp(20),dp(18));c.setBackground(outline(Color.rgb(25,35,43),LINE,1,21));margins(c,0,8,0,10);return c;}
    ...
    Button primary(String s){return button(s,COBALT,WHITE,0);} Button secondary(String s){return button(s,Color.rgb(34,45,53),WHITE,LINE);} Button danger(String s){return button(s,Color.rgb(74,45,48),Color.rgb(255,211,214),Color.rgb(112,62,67));}
    EditText input(String hint,boolean pw){EditText e=new EditText(this);e.setHint(hint);e.setTextSize(16);e.setSingleLine(true);e.setTextColor(INK);e.setHintTextColor(Color.rgb(118,139,143));e.setPadding(dp(15),dp(13),dp(15),dp(13));e.setBackground(outline(Color.rgb(18,27,34),LINE,1,13));if(pw)e.setInputType(InputType.TYPE_CLASS_TEXT|InputType.TYPE_TEXT_VARIATION_PASSWORD);margins(e,0,5,0,9);return e;}
    ...
    void hero(LinearLayout r,String eyebrow,String title,String subtitle,int accent){
        ...
        ImageView logo=new ImageView(this);logo.setImageResource(R.drawable.pangi_mark);logo.setScaleType(ImageView.ScaleType.CENTER_INSIDE);LinearLayout mark=new LinearLayout(this);mark.setGravity(Gravity.CENTER);mark.setBackground(shape(Color.rgb(27,38,46),17));mark.addView(logo,new LinearLayout.LayoutParams(dp(42),dp(42)));brand.addView(mark,new LinearLayout.LayoutParams(dp(52),dp(52)));
        LinearLayout names=new LinearLayout(this);names.setOrientation(LinearLayout.VERTICAL);names.setPadding(dp(12),0,0,0);names.addView(text("PANGI",22,true,WHITE));names.addView(text("20.5 · Observa · Protege · Responde",12,false,Color.rgb(221,230,255)));brand.addView(names);h.addView(brand);
    ...
    LinearLayout role(String badge,String title,String desc,int accent){
        LinearLayout box=new LinearLayout(this);box.setOrientation(LinearLayout.HORIZONTAL);box.setGravity(Gravity.CENTER_VERTICAL);box.setPadding(dp(17),dp(16),dp(15),dp(16));box.setBackground(outline(Color.rgb(25,35,43),LINE,1,19));margins(box,0,6,0,6);
    ...
    void roleGate(){
        ...
        TextView privacy=text("Supervisión en vivo, no grabación histórica. PANGI jamás registra contraseñas ni el texto que escribes.",12,false,MUTED);privacy.setPadding(dp(3),dp(10),dp(3),dp(12));r.addView(privacy);
    ...
    void incompatibleUi(){
        ScrollView sc=shell();LinearLayout r=root();sc.addView(r);hero(r,"COMPATIBILIDAD","Este modelo necesita revisión","PANGI no iniciará sesiones compartidas si Android no ofrece las capacidades de aislamiento necesarias.",CORAL);
    ...
    void prepareSession(String role,String name,String course){
        ...
        },"PangiCreateUser").start();
    }

    void preparingUi(String role,String name,String course){
        ScrollView sc=shell();LinearLayout r=root();sc.addView(r);hero(r,"PREPARANDO TU ESPACIO","Un momento, "+first(name),"PANGI está creando un usuario Android temporal y aplicando protección institucional.",VIOLET);
    ...
    void guestStartUi(String warning){
        ...
        TextView p=text("Android mostrará su autorización oficial de captura. PANGI no intenta saltar ni ocultar este consentimiento.",12,false,MUTED);p.setPadding(dp(3),dp(9),dp(3),0);r.addView(p);
    ...
    void endGuest(){SessionState.set(this,SessionState.State.CLOSING);Core.supervisionStarted(this,false);Core.remoteScreenUntil(this,0);stopService(new Intent(this,ScreenCaptureService.class));Managed.homeGuardOn(this);Managed.enterGate(this);closingUi();ui.postDelayed(()->new Thread(()->SessionUsers.logoutGuest(this),"PangiLogout").start(),500);}
    ...
    void adminPanel(){
        ...
        Button settings=secondary("Ajustes completos de Android"),usage=secondary("Acceso de uso / inactividad"),play=secondary("Abrir Google Play"),apps=secondary("Administrar aplicaciones"),clean=secondary("Limpiar sesiones temporales residuales"),cfg=secondary("Configuración de PANGI"),reset=danger("Restablecer tablet de fábrica"),release=danger("Liberar Device Owner"),done=primary("Cerrar administración y restaurar protección");r.addView(settings);r.addView(usage);r.addView(play);r.addView(apps);r.addView(clean);r.addView(cfg);r.addView(reset);r.addView(release);r.addView(done);
