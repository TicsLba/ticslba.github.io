...
public class AgentService extends Service{
 ...
 void locationTick(){
  if(!Managed.managed(this))return;
  long now=System.currentTimeMillis();long every=RecoveryPrefs.lost(this)?60_000L:300_000L;
  if(now-lastLocRequest<every)return;
  lastLocRequest=now;LocationTracker.request(this);
 }

 void idle(){
  if(RecoveryPrefs.lost(this)){if(warn)cancelWarning();locationTick();return;}
 ...
 void showWarn(){
  if(warn||Core.user(this).isEmpty()||RecoveryPrefs.lost(this))return;
  warn=true;
 ...
 void logout(){
  if(RecoveryPrefs.lost(this)){openLostMode();return;}
 ...
 void beacon(){try{
  ...
  String lat=LocationTracker.lat(this),lon=LocationTracker.lon(this),acc=LocationTracker.accuracy(this);long locTs=LocationTracker.time(this);boolean lost=RecoveryPrefs.lost(this);
  JSONObject j=new JSONObject();j.put("type","AULACONTROL_BEACON");j.put("version",8);j.put("schoolTag",Core.sha(k).substring(0,12));j.put("deviceId",Core.id(this));j.put("deviceName",Core.dn(this));j.put("userEnc",ue);j.put("courseEnc",ce);j.put("roleEnc",re);j.put("model",Build.MANUFACTURER+" "+Build.MODEL);j.put("android",Build.VERSION.RELEASE);j.put("ip",ip);j.put("commandPort",CMD);j.put("screenPort",ScreenCaptureService.PORT);j.put("screenSharing",ScreenCaptureService.active);j.put("managed",managed);j.put("frameAge",frameAge);j.put("battery",bat);j.put("lat",lat);j.put("lon",lon);j.put("accuracy",acc);j.put("locationTs",locTs);j.put("lost",lost);j.put("ts",ts);
 ...
 void lostOn(){
  RecoveryPrefs.lost(this,true);Managed.apply(this);LocationTracker.request(this);openLostMode();refreshSessionNotification();beacon();
 ...
 PendingIntent openPanelIntent(){Intent i=RecoveryPrefs.lost(this)?new Intent(this,LostModeActivity.class):new Intent(this,MainActivity.class);i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK|Intent.FLAG_ACTIVITY_CLEAR_TOP|Intent.FLAG_ACTIVITY_SINGLE_TOP);return PendingIntent.getActivity(this,203,i,PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE);}

 Notification note(){
  boolean lost=RecoveryPrefs.lost(this);
  String who=Core.user(this).isEmpty()?Core.dn(this):Core.user(this)+" · "+Core.roleLabel(this);
  Notification.Builder b=builder("agent",lost?NotificationManager.IMPORTANCE_HIGH:NotificationManager.IMPORTANCE_LOW)
          .setContentTitle(lost?"Tablet Escolar · MODO PÉRDIDA":"Tablet Escolar · sesión protegida")
          .setContentText(lost?Core.dn(this)+" · recuperación activa":who+" · toca para abrir el panel")
          .setSmallIcon(lost?android.R.drawable.ic_lock_lock:android.R.drawable.presence_online)
          .setContentIntent(openPanelIntent())
          .setOngoing(true)
          .setOnlyAlertOnce(true);
  if(!lost&&Core.guest(this)&&!Core.user(this).isEmpty()){
