...
public class ScreenCaptureService extends Service {
    ...
    Notification note(){NotificationManager nm=(NotificationManager)getSystemService(NOTIFICATION_SERVICE);String c="capture";if(Build.VERSION.SDK_INT>=26&&nm.getNotificationChannel(c)==null)nm.createNotificationChannel(new NotificationChannel(c,"Tablet Escolar · supervisión",NotificationManager.IMPORTANCE_LOW));Notification.Builder b=Build.VERSION.SDK_INT>=26?new Notification.Builder(this,c):new Notification.Builder(this);Intent open=new Intent(this,MainActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK|Intent.FLAG_ACTIVITY_CLEAR_TOP|Intent.FLAG_ACTIVITY_SINGLE_TOP);PendingIntent pi=PendingIntent.getActivity(this,301,open,PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE);return b.setContentTitle("Tablet Escolar · supervisión activa").setContentText("Supervisión en tiempo real, no vigilancia histórica").setSmallIcon(android.R.drawable.presence_online).setContentIntent(pi).setOngoing(true).setOnlyAlertOnce(true).build();}
}
