...
public class AttentionActivity extends Activity{
 ...
 @Override public void onCreate(Bundle b){super.onCreate(b);getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);getWindow().setStatusBarColor(NAVY);getWindow().setNavigationBarColor(NAVY);LinearLayout r=new LinearLayout(this);r.setOrientation(LinearLayout.VERTICAL);r.setGravity(Gravity.CENTER);r.setPadding(dp(34),dp(34),dp(34),dp(34));r.setBackgroundColor(NAVY);
  LinearLayout mark=new LinearLayout(this);mark.setGravity(Gravity.CENTER);mark.setBackground(shape(BLUE,22));ImageView icon=new ImageView(this);icon.setImageResource(R.drawable.ic_tablet_school_mark);mark.addView(icon,new LinearLayout.LayoutParams(dp(62),dp(62)));r.addView(mark,new LinearLayout.LayoutParams(dp(78),dp(78)));
  TextView a=t("Tablet Escolar",18,true);a.setTextColor(MUTED);a.setPadding(0,dp(18),0,0);r.addView(a);
  ...
  TextView n=t("Esta pausa fue activada desde la consola docente.",13,false);n.setTextColor(MUTED);n.setPadding(0,dp(28),0,0);r.addView(n);setContentView(r);registerReceiver(off,new IntentFilter("cl.antumapu.aulacontrol.ATTENTION_OFF"),RECEIVER_NOT_EXPORTED);}
