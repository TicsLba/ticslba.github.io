...
import android.app.*;import android.app.usage.*;import android.content.*;import android.os.*;import android.provider.Settings;import android.util.Base64;
...
final class Core {
 static final String P="aulacontrol", ROLE_STUDENT="student", ROLE_TEACHER="teacher", ROLE_ADMIN="admin"; private Core(){}
 ...
 static String user(Context c){return dec(c,"user");} static String course(Context c){return dec(c,"course");} static String role(Context c){String r=sp(c).getString("role","");return r==null?"":r;}
 static boolean guest(Context c){return sp(c).getBoolean("guest_session",false);} static boolean adminMode(Context c){return sp(c).getBoolean("admin_mode",false);} 
 ...
 static void setup(Context c,String dn,String key,String pw){String salt=salt(),hash=hash(pw,salt);sp(c).edit().putString("dn",dn.trim()).putString("key_enc",Secrets.encrypt(key.trim())).remove("key").putString("salt",salt).putString("hash",hash).apply();}
 static void setupGuest(Context c,PersistableBundle b){if(b==null)return;String k=b.getString("key","");sp(c).edit().putBoolean("guest_session",true).putString("dn",b.getString("deviceName","AulaControl")).putString("id",b.getString("deviceId",id(c))).putString("key_enc",Secrets.encrypt(k)).putString("salt",b.getString("adminSalt","")).putString("hash",b.getString("adminHash","")).putString("role",b.getString("role",ROLE_STUDENT)).putString("user_enc",Secrets.encrypt(b.getString("name",""))).putString("course_enc",Secrets.encrypt(b.getString("course",""))).putLong("last",System.currentTimeMillis()).apply();}
 static void config(Context c,String dn,String key,String pw){var e=sp(c).edit().putString("dn",dn.trim()).putString("key_enc",Secrets.encrypt(key.trim())).remove("key");if(!pw.isEmpty()){String s=salt();e.putString("salt",s).putString("hash",hash(pw,s));}e.apply();}
 static String adminSalt(Context c){return sp(c).getString("salt","");}static String adminHash(Context c){return sp(c).getString("hash","");}
 static boolean checkPw(Context c,String pw){var s=sp(c);return eq(s.getString("hash",""),hash(pw,s.getString("salt","")));}
 static void login(Context c,String u,String co,String role){sp(c).edit().putString("user_enc",Secrets.encrypt(u.trim())).putString("course_enc",Secrets.encrypt(co==null?"":co.trim())).putString("role",role).putLong("last",System.currentTimeMillis()).apply();}
 static void clearIdentity(Context c){sp(c).edit().remove("user_enc").remove("course_enc").remove("user").remove("course").remove("role").remove("last").remove("admin_mode").apply();}
 ...
 static String sealText(String key,String plain){return Base64.encodeToString(seal(key,(plain==null?"":plain).getBytes(StandardCharsets.UTF_8)),Base64.NO_WRAP);} static String openText(String key,String enc){try{return new String(open(key,Base64.decode(enc,Base64.NO_WRAP)),StandardCharsets.UTF_8);}catch(Exception e){return "";}}
