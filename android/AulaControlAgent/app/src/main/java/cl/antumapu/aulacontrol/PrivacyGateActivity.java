...
public class PrivacyGateActivity extends Activity{
 ...
 static final String ACCEPT="privacy_v120";
 @Override public void onCreate(Bundle b){super.onCreate(b);if(getPreferences(MODE_PRIVATE).getBoolean(ACCEPT,false)){go();return;}showNotice();}
 ...
 void showNotice(){ScrollView sc=new ScrollView(this);LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(22),dp(26),dp(22),dp(28));root.setBackgroundColor(BG);sc.addView(root);
  ...
  add(card,"✓ Sesión temporal", "Al cerrar sesión se elimina la identidad local, se detiene la supervisión y AulaControl inicia un restablecimiento de datos temporales compatibles antes del siguiente usuario.");
  add(card,"✓ Sin historial personal por defecto", "La consola no necesita conservar nombres anteriores para operar. La versión 1.2 registra únicamente eventos técnicos de sesión sin guardar el nombre ni curso en el historial persistente.");
