...
public class PrivacyGateActivity extends Activity{
 ...
 void showNotice(){
  ...
  LinearLayout names=new LinearLayout(this);names.setOrientation(LinearLayout.VERTICAL);names.setPadding(dp(14),0,0,0);names.addView(t("Aula Móvil",26,true,NAVY));names.addView(t("Privacidad y seguridad de la sesión",13,false,MUTED));brand.addView(names);root.addView(brand);
  ...
  TextView foot=t("Aula Móvil 4.0 · Gestión de aula y dispositivos",12,false,MUTED);foot.setGravity(Gravity.CENTER);foot.setPadding(0,dp(12),0,0);root.addView(foot);setContentView(sc);
 }
